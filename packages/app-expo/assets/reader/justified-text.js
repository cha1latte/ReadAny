(function installReadAnyJustifiedText(root) {
  ...
  function apply(doc, enabled, unsupportedLayout, policy) {
    ...
    // Preserved alignments may need an explicit override so a justified
    // parent cannot change their originally inherited alignment.
    const elements = [];
    ...
    for (const { element, alignment, prose } of elements) {
      ...
      const target = prose && !PRESERVED_ALIGNMENTS.has(alignment) ? "justify" : alignment;
      // Preserve non-prose and intentional alignment without writes unless a modified
