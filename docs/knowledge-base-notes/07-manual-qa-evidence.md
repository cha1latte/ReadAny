...
| --- | --- | --- | --- |
| Clean branch | Worktree is clean and pushed before manual QA starts. | Pass | Runtime commit `e296f513` was tested by the automated gate; this manual evidence refresh is docs-only and should be pushed before manual QA starts. |
| Full automated gate | `pnpm acceptance:knowledge` passes. | Pass | Passed on 2026-06-15 for commit `e296f513`: 33 core test files / 488 tests plus core TS, desktop TS, desktop production bundle, mobile TS, WebView bundle, contract checks including persistent proposal-apply failure states with localized conflict reasons, grouped custom-card field sections, AI tool write-safety status, AI search match-field explanations, vault import conflict resolution guidance, and whitespace check. |
