...
| Failed tool calls become visible failure cards with tool names, reasons, no-write hints, and available vault paths instead of endless loading states. | `tool-call-state.test.ts`, `tool-result.test.ts`, `knowledge-tool-result.test.ts`, desktop production bundle contract check, mobile AI knowledge chat contract check |
