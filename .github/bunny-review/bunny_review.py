...
def guidance_from_rules(files, rules):
    guidance = ["docs/readany-shlai/development.md"]
...
def select_guidance(files):
    ...
    return ["docs/readany-shlai/development.md"]
...
def normalize_review_object(review_obj, base, files):
    ...
    if used_fallback:
        note = (
            "Bunny rebuilt the change summary from git diff metadata because the final model output omitted `change_summary`."
        )
        if note not in review_obj["what_i_checked"]:
            review_obj["what_i_checked"].insert(0, note)
    return review_obj
...
def issue_comments(pr_num):
    ...
    return [
        comment for comment in load_json_list(gh.stdout)
...
def missing_model_api_key_message():
    return (
        ...
        "workflow run. Set `SHLAI_BUNNY_LLM_API_KEY` for provider-specific Bunny "
...
def post_review(args):
    ...
    latest_head = run_gh(
        ["pr", "view", pr_num, "--json", "headRefOid", "-q", ".headRefOid"], check=True
    ).stdout.strip()
    if head_sha != latest_head:
        raise RuntimeError("PR head changed during review; refusing to post stale findings.")
