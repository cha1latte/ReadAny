...
- Make `getAnnotations` available to the reading agent's `current_chapter_context` route. Localized prompts containing a recognized current-chapter cue, including Chinese “这一章”, are intentionally classified through that route.
...
3. The reading agent classifies each localized sentence using its existing routing rules. English currently uses the broad book route, while recognized localized chapter cues such as Chinese “这一章” use `current_chapter_context`.
